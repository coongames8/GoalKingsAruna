...
export default function Tips({ userData }) {
  ...
  const [insightsPerPage] = useState(25);
  ...
  useEffect(() => {
    getTips(insightsPerPage, setInsights, setLoading, formatDate(currentDate));
  }, [isOnline, insightsPerPage, currentDate]);
  ...
  return (
    <div className="tips">
      <AppHelmet title={"Goal Kings"} location={"/"} />
      <div className="container">
        ...
        <div className="tips-grid">
          {insights.length > 0 ? (
            ...
            <h1>No Matches Found For This Date!</h1>
          )}
        </div>

        <div className="wrapper">
          {!isOnline && insights.length === 0 && !loading && (
          ...
          {!insights.length > 0 && loading && <Loader />}
        </div>
      </div>
